<?php 
    ...
    if (!isset($_SESSION['user']))
        header("Location: http://107.15.138.161/login.php");
    $search_err = "";
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if (isset($_POST['search'])) {
            ...
            if ($userExists){
                header("Location: http://107.15.138.161/profile.php/?profile=" . trim($_POST["search"]));
            } else {
    ...
    function findImg($user) {
        ...
        for ($a = 0; $a < count($imgTypes); $a++) {
            if (file_exists('pfps/' . $user . $imgTypes[$a]))
                return 'http://107.15.138.161/pfps/' . $user . $imgTypes[$a];
        }
    }
?>
...
<div class="w3-top">
    <div class="w3-bar w3-theme-d2 w3-left-align w3-large">
        ...
        <a href="http://107.15.138.161/" class="w3-bar-item w3-button w3-padding-large w3-theme-d4"><i class="fa fa-home w3-margin-right"></i></a>
        ...
        <a href="<?php echo 'http://107.15.138.161/profile.php/?profile=' . $_SESSION['user'];?>" class="w3-round sw3-bar-item w3-button w3-hide-small w3-right w3-padding-large w3-hover-white" title="My Account">
